...
const portfolioImages: { [key: string]: string } = {
  'digital marketing website': '/portfolio/digital-marketing-website.png',
  'cloud website': '/portfolio/cloud-website.png',
...
const getCategoryGradient = (title: string, category: string): { gradient: string; pattern: string } => {
  const titleLower = (title || '').toLowerCase()
  const categoryLower = (category || '').toLowerCase()

  if (titleLower.includes('digital marketing') || titleLower.includes('marketing') || categoryLower.includes('marketing')) {
    return { gradient: 'from-blue-500 via-cyan-500 to-teal-500', pattern: '📊' }
  }
  if (titleLower.includes('e-commerce') || titleLower.includes('ecommerce') || titleLower.includes('shop')) {
    return { gradient: 'from-orange-500 via-red-500 to-pink-500', pattern: '🛒' }
  }
  if (titleLower.includes('restaurant') || titleLower.includes('food')) {
    return { gradient: 'from-yellow-500 via-orange-500 to-red-500', pattern: '🍽️' }
  }
  if (titleLower.includes('portfolio') || titleLower.includes('personal')) {
    return { gradient: 'from-purple-500 via-pink-500 to-rose-500', pattern: '💼' }
  }
  if (titleLower.includes('real estate') || titleLower.includes('property')) {
    return { gradient: 'from-green-500 via-emerald-500 to-teal-500', pattern: '🏠' }
  }
  if (titleLower.includes('healthcare') || titleLower.includes('medical') || titleLower.includes('hospital')) {
    return { gradient: 'from-blue-600 via-indigo-600 to-purple-600', pattern: '🏥' }
  }
  if (titleLower.includes('education') || titleLower.includes('school') || titleLower.includes('course') || titleLower.includes('learning')) {
    return { gradient: 'from-indigo-500 via-purple-500 to-pink-500', pattern: '📚' }
  }
  if (titleLower.includes('fitness') || titleLower.includes('gym') || titleLower.includes('health')) {
    return { gradient: 'from-red-500 via-orange-500 to-yellow-500', pattern: '💪' }
  }
  if (titleLower.includes('travel') || titleLower.includes('tourism') || titleLower.includes('hotel')) {
    return { gradient: 'from-sky-500 via-blue-500 to-indigo-500', pattern: '✈️' }
  }
  if (titleLower.includes('app') || titleLower.includes('mobile')) {
    return { gradient: 'from-violet-500 via-purple-500 to-fuchsia-500', pattern: '📱' }
  }
  if (titleLower.includes('registrar') || titleLower.includes('panel') || titleLower.includes('dashboard')) {
    return { gradient: 'from-slate-600 via-gray-600 to-zinc-600', pattern: '⚙️' }
  }
  if (categoryLower.includes('web development') || categoryLower.includes('website')) {
    return { gradient: 'from-blue-600 via-indigo-600 to-purple-600', pattern: '💻' }
  }
  if (categoryLower.includes('digital marketing')) {
    return { gradient: 'from-cyan-500 via-blue-500 to-indigo-500', pattern: '📈' }
  }
  if (categoryLower.includes('ui') || categoryLower.includes('ux') || categoryLower.includes('design')) {
    return { gradient: 'from-pink-500 via-rose-500 to-red-500', pattern: '🎨' }
  }
  if (categoryLower.includes('seo')) {
    return { gradient: 'from-green-600 via-emerald-600 to-teal-600', pattern: '🔍' }
  }

  return { gradient: 'from-purple-600 via-pink-600 to-blue-600', pattern: '🌐' }
}
...
export default function Portfolio() {
  ...
  const categories = ['all', ...Array.from(new Set(portfolio.map(item => item.category || 'Uncategorized')))]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {filteredPortfolio.length === 0 ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
          >
            <p className="text-white/50 text-lg">No portfolio items available</p>
          </motion.div>
        ) : (
          <motion.div
          ...
          >
            <AnimatePresence>
              {filteredPortfolio.map((item, index) => {
                const { gradient, pattern } = getCategoryGradient(item.title || '', item.category || '')
                const localImagePath = getLocalImagePath(item.title || '')
                const hasRealImage = localImagePath || (item.image_url && item.image_url.trim() !== '')
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <motion.div
                          className="relative p-6 translate-y-[calc(100%-4rem)] group-hover:translate-y-0 transition-transform duration-500 ease-out"
                        >
                          ...
                          <motion.p
                            className="text-white/80 text-sm mb-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100 line-clamp-3"
                          >
        ...
        {selectedItem && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              {(getLocalImagePath(selectedItem.title || '') || selectedItem.image_url) ? (
                <div className="relative h-96 w-full">
                  <Image
                    src={getLocalImagePath(selectedItem.title || '') || selectedItem.image_url || ''}
                ...
                <div className={`h-96 w-full bg-gradient-to-br ${getCategoryGradient(selectedItem.title || '', selectedItem.category || '').gradient} flex items-center justify-center`}>
                  <span className="text-8xl opacity-30">
                    {getCategoryGradient(selectedItem.title || '', selectedItem.category || '').pattern}
                  </span>
                </div>
              )}
              <div className="p-8">
